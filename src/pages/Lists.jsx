...
export default function ListsPage() {
  ...
  const loadLists = async (userId) => {
    try {
      ...
      const fetchedLists = await PackingList.filter(
        { owner_id: userId },
        "-created_date"
      );
  ...
  const filteredLists = lists.filter(list => {
    const matchesName = list.name ? 
      list.name.toLowerCase().includes(searchQuery.toLowerCase()) : false;
      
    const matchesDestination = list.destinations && list.destinations.length > 0 && list.destinations[0].location
    ...
    const matchesTab = activeTab === "all" || 
      (activeTab === "favorites" && list.is_favorite);
    ...
    <div className="p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <h1 className="text-2xl font-bold" onClick={() => navigate(createPageUrl("Home"))}>My Packing Lists</h1>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-gray-500 mt-1">
                      {format(getDateRange(list).start, "MMM d")} - {format(getDateRange(list).end, "MMM d, yyyy")}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span className="text-sm">
                        {list.items ? `${list.items.filter(item => item.is_packed).length} / ${list.items.length} items packed` : '0 items'}
                      </span>
                    </div>
                    <Button 
                      variant="outline" 
