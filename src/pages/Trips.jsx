...
export default function TripsPage() {
  ...
  const loadTrips = async userId => {
    try {
      ...
      const fetchedTrips = await PackingList.filter({ owner_id: userId }, '-created_date');
  ...
  const filteredTrips = trips.filter(trip => {
    ...
    const matchesDestination =
      trip.destinations && trip.destinations.length > 0 && trip.destinations[0].location
    ...
    const matchesTab = activeTab === 'all' || (activeTab === 'favorites' && trip.is_favorite);
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-gray-500 mt-1">
                      {format(getDateRange(trip).start, 'MMM d')} -{' '}
                      {format(getDateRange(trip).end, 'MMM d, yyyy')}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span className="text-sm">
                        {trip.items
                          ? `${trip.items.filter(item => item.is_packed).length} / ${trip.items.length} items packed`
                          : '0 items'}
                      </span>
                    </div>
                    <Button
